...
private:
    ...
    // --- ClawBridge: TCP remote event injection ---
public:
    void cbInjectGamepad(unsigned int buttonFlag, unsigned char action);
    void cbInjectKeyboard(short vk, unsigned char action, char mods);
    void cbInjectMouseButton(unsigned char button, unsigned char action);
    void cbInjectScroll(short dx, short dy);
    void cbInjectMouseMove(short dx, short dy);
    void cbInjectMousePosition(short x, short y, short w, short h);
    void cbInjectJoystick(short lx, short ly, short rx, short ry,
                          unsigned char lt, unsigned char rt);
    void cbInjectText(const char* utf8str);

private:
    ClawBridgeServer* m_cmdServer = nullptr;
};
