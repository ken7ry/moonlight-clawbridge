...
// ================================================================
// Safe push — no copy constructors, payload is constructed in-place
// ================================================================

// Generic cleanup for trivially-copyable payloads
template<typename T>
static void cleanupPayload(T* p) { delete p; }

// Specialization for CBEvt_Txt — must delete[] the string first
template<>
...
// Generic push for trivial payloads (CBEvt_Gp, Kb, Mb, Wh, Mv, Mp)
...
// Specialized push for CBEvt_Txt — takes ownership of the raw char*
// No copy constructor, no intermediate duplication
static void pushText(CB_Uint32 evType, char* str)
{
    CBEvt_Txt* heap = new CBEvt_Txt(str);  // direct ownership, no copy
    SDL_Event ev = {};
    ev.type         = static_cast<Uint32>(evType);
    ev.user.code    = 0;
    ev.user.data1   = static_cast<void*>(heap);
    ev.user.data2   = nullptr;
    if (SDL_PushEvent(&ev) < 0) {
        cleanupPayload(heap);  // calls delete[] str + delete p
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
...
void ClawBridgeServer::onDisconnect()
{
    ...
    if (sock) {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                    "[ClawBridge] Client disconnected: %s",
                    qUtf8Printable(sock->peerAddress().toString()));
        sock->disconnect();
        sock->deleteLater();
...
void ClawBridgeServer::parseLine(const QString& cmd)
{
    ...
    if (cmd.startsWith("GP:")) {
        ...
        if (part.endsWith("_PRESS"))       { pressed = true;  part.chop(6); }
            ...
            SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                        "[ClawBridge] GP: requires _PRESS or _RELEASE suffix");
            return;
        }
    ...
    if (cmd.startsWith("KB:")) {
        ...
        if (part.endsWith("_DOWN"))      { pressed = true;  part.chop(5); }
            ...
            SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                        "[ClawBridge] KB: requires _DOWN or _UP suffix");
            return;
        }
        CB_short vk = nameToKeyCode(part);
        if (vk == 0) return;
        // mods = 0 — pure background automation, no host modifier state
    ...
    if (cmd.startsWith("MB:")) {
        ...
        if (part.endsWith("_PRESS"))       { pressed = true;  part.chop(6); }
            ...
            SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                        "[ClawBridge] MB: requires _PRESS or _RELEASE suffix");
            return;
        }
    ...
    if (cmd.startsWith("MV:")) {
        ...
        if (nums.size() == 2) {
            ...
            SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                        "[ClawBridge] MV: format is MV:dx,dy");
        }
        return;
    }
    ...
    if (cmd.startsWith("MP:")) {
        ...
        if (nums.size() == 4) {
            ...
            SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                        "[ClawBridge] MP: format is MP:x,y,screenW,screenH");
        }
        return;
    }
    ...
    // ----- Text Input — direct char* ownership, no copy -----
    if (cmd.startsWith("TXT:")) {
        ...
        pushText(CB_EV_TEXT, copy);  // ownership transferred
        return;
    }
